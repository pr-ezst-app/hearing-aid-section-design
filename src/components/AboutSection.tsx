...
export default function AboutSection() {
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-[600px] h-[600px] rounded-full bg-[#EAF2F8] opacity-40 translate-x-1/3 -translate-y-1/4 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full bg-[#2A7B6F] opacity-5 -translate-x-1/2 translate-y-1/3 pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <span
            className="text-[#C4975A] text-sm tracking-[0.2em] uppercase font-dm font-medium"
          >
            About Our Clinic
          </span>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          {/* LEFT — Text content */}
          <div>
            <h2 className="font-cormorant text-5xl lg:text-6xl font-semibold leading-[1.1] text-[#1B2B4B] mb-6">
              Decades of Expertise,{" "}
              <em className="italic text-[#2A7B6F]">One Mission</em>
              <br />— Better Hearing.
            </h2>
            ...
            <div className="space-y-6">
              {values.map((v) => (
                <div key={v.title} className="flex gap-4 items-start group">
                  <div className="flex-shrink-0 w-11 h-11 rounded-xl bg-white shadow-sm border border-[#E2E8F0] flex items-center justify-center group-hover:bg-[#2A7B6F] transition-colors duration-300">
                    <Icon
                      name={v.icon}
                      size={20}
                      className="text-[#2A7B6F] group-hover:text-white transition-colors duration-300"
                    />
                  </div>
                  <div>
                    <h4 className="font-dm font-semibold text-[#1B2B4B] mb-1">{v.title}</h4>
            ...
            <div className="mt-10 flex items-center gap-6">
              <button className="font-dm font-medium px-7 py-3.5 bg-[#1B2B4B] text-white rounded-lg hover:bg-[#2A3F6B] transition-colors duration-200 text-sm tracking-wide">
                Meet Our Team
              </button>
              <button className="font-dm font-medium text-[#2A7B6F] text-sm flex items-center gap-2 hover:gap-3 transition-all duration-200">
                Book a Consultation
          ...
          <div className="relative h-[580px] lg:h-[680px]">
            ...
            <div className="absolute top-0 right-0 w-[75%] h-[72%] rounded-2xl overflow-hidden shadow-2xl">
              ...
              />
              {/* Subtle overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#1B2B4B]/20 to-transparent" />
            </div>
            ...
            <div className="absolute top-6 left-4 bg-white rounded-2xl shadow-lg px-5 py-4 border border-[#E2E8F0]">
              <p className="font-cormorant text-4xl font-bold text-[#1B2B4B] leading-none">20+</p>
              <p className="font-dm text-xs text-[#718096] mt-1 leading-tight">Years of<br/>trusted care</p>
            </div>

            {/* Floating badge — rating */}
            <div className="absolute bottom-28 right-4 bg-[#2A7B6F] rounded-2xl shadow-lg px-4 py-3 flex items-center gap-3">
